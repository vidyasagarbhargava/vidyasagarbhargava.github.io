...
## Let's Code Neural Machine Translation

In the below colab notebook we will code neural machine translation and leverage the free gpu from google. Click on colab logo.
...
## Challenge with encoder decoder

Here challenge come along with the long sentences where the context vector turn out to be bottleneck for these types of models.Then a paper was introduced and refined a technique called **Attention**, which highly improved the quality of machine translation systems.

> Attention allows the model to focus on the relevant part of the input sequences as needed.

An attention model differs from classic seq-to-seq model in two ways :

-   The encoder passes alot more data to decoder. Instead of passing the last hidden state, now it passes all the hidden states to the decoder.
...
The scoring excercise is done at each time step on the decoder side.
