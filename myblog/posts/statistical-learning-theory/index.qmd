...
Input space : $R^d$ , Action space : $R$ , Output space : $R$

What are the spaces for logistic regression ?

Input space: $R^d$ , Action space: Probability between 0 & 1, Output space: \[0,1\]
:::
...
-   Can't compute $R(f) = El(f(x),y)$ because we don't know $P_{x \times y}$ because no one tells us the probability of distribution describing the real world.
...
$$
\lim_{n\to\infty} \frac{1}{n} \sum_{i=1}^{n} Z_{i} = Ez
$$
